...
class RandomForest extends Classifier {
  ...
  override def train(input: DStream[Example]): Unit = {
    ...
    var count = 0
    val startTime = System.currentTimeMillis().toDouble/1000
    input.foreachRDD {
      rdd =>
        count+=1
...
class RandomForestSequential extends Classifier {

  type T = RandomForestModel

  val numTreeObsOption: IntOption = new IntOption("numTreeObs", 'j',
    "Number of Observations per Tree", 100000, 1, Int.MaxValue)
  ...
  override def train(input: DStream[Example]): Unit = {
    ...
    input.foreachRDD {
      rdd =>
        count+=1
        var trainingStartTime = System.currentTimeMillis().toDouble/1000
        val i = rdd.count().toInt*count/numTreeObsOption.getValue
        if (i<numTrees) {
          val tmodel = rdd.aggregate(
            new HoeffdingTreeModel(model.trees(i)))(
            (mod, example) => {
  ...
  def init(): Unit = {
    ...
    for (i <- 0 until numTrees) {
      trees(i) = new HoeffdingTreeModel(espec, numericObserverType, splitCriterion,
        ...
        noPrePrune, removePoorFeatures, splitAll, decorrelation
      )
      trees(i).init()
    }
  }
